...
#acc_fn=np.zeros((int(np.size(label,0)/batch_size)*(num_steps+1)));
num_input =28*28  # MNIST data input (img shape: 28*28)
num_classes = 4 # MNIST total classes (0-9 digits)
dropout = 0.01 # Dropout, probability to keep units
...
loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=Y))+0.001*tf.nn.l2_loss(weights['wc1']) +0.001*tf.nn.l2_loss(weights['wc2'])+0.001*tf.nn.l2_loss(weights['wd1']) +0.001*tf.nn.l2_loss(weights['out'])
...
with tf.Session() as sess:
    ...
    train=np.load('training_40ensemble_4classes.npy')
#    train=trainfull[:,range(0,4000)]
    label=np.load('labels_40ensemble_4classes.npy')
#    label=labelfull[range(0,4000),:]
#    loss_fn=np.zeros((int(np.size(label,0)/batch_size)*(num_steps+1)));
#    acc_fn=np.zeros((int(np.size(label,0)/batch_size)*(num_steps+1)));
    loss_fn=[]
    acc_fn=[]
    for step in range(1, num_steps+1):
      ...
      for step2 in range(0,int(np.size(label,0)/batch_size)):
        ...
        if step % display_step == 0 or step == 1:
          ...
          #  acc_fn[count]=acc
          #  count=count+1
          #  loss_fn=np.append(loss_fn,loss)
          #  acc_fn=np.append(acc_fn,acc)
    ...
    test_x=np.load('test_40ensemble_4classes.npy')
...
#    np.savetxt("prediction_TRUE_4layers.csv", pred, delimiter=",")
        
    np.savetxt("loss_fulldata_bad.csv", loss_fn, delimiter=",")
    np.savetxt("acc_fulldata_bad.csv", acc_fn, delimiter=",")
